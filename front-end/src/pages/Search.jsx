...
const Search = () => {
  ...
  const { isLoading, search } = useProgress();
        ...
        if (searchTerm) {
          ...
          await Promise.all(result.docs.map(async (book) => {
            ...
            try {
              ...
              if (descriptionData.description && typeof descriptionData.description === 'string') {
                const cleanDescription = descriptionData.description.replace(/source|: https?:\/\/\S+|\(\[\]\[1\]\) \[1\]/g, '').trim();
              ...
              <li className="mx-12 mt-8 flex gap-16 border border-gray-300 rounded-lg shadow justify-between p-6">
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <div className="flex justify-center mt-4">
                    ...
                    >
                      {openItem === id ? 'Hide Description' : 'Show Description'}
                    </button>
                  </div>
                </div>
                <div className="flex flex-col justify-center bottom-4 right-4">
                  <button onClick={() => handleAdd(item, id)} type="button" className="focus:outline-none text-white bg-green-400 hover:bg-green-800  focus:ring-green-300 focus:bg-green-800 active:bg-green-800 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-200 dark:hover:bg-green-700 dark:focus:ring-green-00">Add book</button>
                </div>
              </li>
            </div>
          );
